...
import {createPost, updatePost} from '../../actions/apost.js';
...
const Form=({currentId, setCurrentId})=>{
    ...
    const epost=useSelector((state)=>currentId?state.rposts.find((x)=>x._id===currentId):null);
    const styless=useStyles();
    const dispatch=useDispatch();
    useEffect(()=>{
        if(epost) setPostData(epost);
    },[epost])
    ...
    return(
        <Paper className={styless.paper}>
            <form autoComplete='off' noValidate className={`${styless.root} ${styless.form}`} onSubmit={handleSubmit}>
                ...
                <Button className={styless.buttonSubmit} variant='contained' color='primary' size='large' 
